...
def get_mail(nombre, key):
	...
	mensaje+='<p>para proceder usa el siguiente <a href="https://www.zaunt2.es/password/%s">link</a> </p>' % key
	mensaje+='<p>si no has solicitado el dicho cambio ignorar este correo </p>'
	mensaje+='<p> Att: Staff <strong>Metin2 Zaunt </strong> </p>'
	return mensaje

def get_mail_register(cuenta, key):
	mensaje = '<h3> Bienvenido a Metin2 Zaunt. </h3>'
	...
	mensaje+= '<p>URL del Sitio: http://www.zaunt2.es/ <p>'
	mensaje+= '----------------------------'
	mensaje+= '<p>Usa el siguiente link para activar tu cuenta: <a href="https://www.zaunt2.es/activar/%s"> Activar </a> </p>' % key
	mensaje+= '----------------------------'
	...
	mensaje+= '--'
	mensaje+= '<p>Att: Staff Metin2 Zaunt </p>'
	return mensaje
